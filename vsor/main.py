...
class DialogoError:
	...
	def __init__(self, padre, titulo, descripcion, detalles=''):
		...
		self.winDialogo.connect('delete-event', lambda s, ev: self.winDialogo.hide() or True)
		self.winDialogo.connect('key-press-event', self.winDialogo_key_press_event)
...
class Aplicacion:
	...
	def __init__(self, archivo=None):
		...
		self.no_imagenes = [] # archivos que definitivamente no son imágenes (fallaron al cargar)
		self.ultima_pos_x = 0
		self.ultima_pos_y = 0
		...
		self.evboxImagen.connect('button-press-event', self.evboxImagen_button_press_event)
		self.evboxImagen.connect('button-release-event', self.evboxImagen_button_release_event)
		self.evboxImagen.connect('motion-notify-event', self.evboxImagen_motion_notify_event)
		...
		self.mbtnEspH.connect('clicked', lambda s: self.espejar(vertical=False))
		...
		self.btnRotarAntiHorario.connect('clicked', lambda s: self.rotar(horario=False))
		self.btnRotarAntiHorario.set_visible(self.BOTONES_ROTACION_EN_BARRA_TITULO)

		self.btnRotarHorario = builder.get_object('btnRotarHorario')
		self.btnRotarHorario.connect('clicked', lambda s: self.rotar(horario=True))
		self.btnRotarHorario.set_visible(self.BOTONES_ROTACION_EN_BARRA_TITULO)

		self.btnRotarAntiHorarioMenu = builder.get_object('btnRotarAntiHorarioMenu')
		self.btnRotarAntiHorarioMenu.connect('clicked', lambda s: self.rotar(horario=False))

		self.btnRotarHorarioMenu = builder.get_object('btnRotarHorarioMenu')
		self.btnRotarHorarioMenu.connect('clicked', lambda s: self.rotar(horario=True))
		...
		self.btnAgregarEtiqueta.connect('clicked', self.btnAgregarEtiqueta_clicked)
		...
		self.btnCopiarEtiquetas.connect('clicked', self.btnCopiarEtiquetas_clicked)
		
		# ventana principal
		self.winPrincipal = builder.get_object('winPrincipal')
		self.winPrincipal.set_wmclass('floating', 'floating')
		self.winPrincipal.connect('key-press-event', self.winPrincipal_key_press_event)
		self.winPrincipal.connect('check-resize', self.winPrincipal_check_resize)
		self.winPrincipal.connect('show',
			lambda s: self.winPrincipal_show(s, archivo) if archivo else None
		)
	...
	def guardar_etiquetas(self):
		...
		setxattr(self.archivonombre_actual, 'user.tags', ','.join(etiquetas).encode())
	...
	def agregar_etiqueta_en_listbox(self, etiqueta:str):
		etiquetas = [ c.get_child().get_children()[0].get_text() for c in self.lbEtiquetas.get_children() ]
	...
	def hallar_imagenes(self, directorio:str):
		...
		return sorted([ a.name for a in archivos if not a.name in self.no_imagenes and any(
			[ a.name.lower().endswith(ext) or not '.' in a.name for ext in self.extensiones_validas ]
		)])
	...
	def cargar_imagen(self, pb, setear=True):
		...
		try:
			...
			DialogoError(self.winPrincipal, 'Error al cargar la imagen', f'El formato de {self.archivonombre_actual} no es soportado.', detalles=e)
			self.limpiar_estado()
	...
	def cargar_imagen_archivo(self, archivo: str, mostrar_error=True) -> bool:
		...
		try:
			...
			try:
				indice_actual = self.lista_imagenes.index(os.path.basename(self.archivonombre_actual))
			...
			self.hb.set_subtitle('(%s/%s) %s' % (indice_actual + 1, cant, os.path.basename(self.archivonombre_actual)))
			...
			etiquetas_archivo = self.obtener_etiquetas(self.archivonombre_actual)
			...
			if mostrar_error:
				DialogoError(self.winPrincipal, 'Error al cargar el archivo', f'El archivo «{archivo}» no se pudo cargar', detalles=str(e))
				self.limpiar_estado()
			else:
	...
	def winPrincipal_key_press_event(self, s, ev):
		if ev.keyval == Gdk.KEY_KP_Add or ev.keyval == Gdk.KEY_plus:
			...
			self.winPrincipal.set_decorated(not self.winPrincipal.get_decorated())
	...
	def winPrincipal_check_resize(self, s):
		if self.ajustar and self.pb_actual and type(self.pb_actual) is Pixbuf:
			...
			if self.pb_actual.get_height() > tamano_box.height:
				width = self.pb_actual.get_width() * tamano_box.height / self.pb_actual.get_height()
				self.imgMain.set_from_pixbuf(self.pb_actual.scale_simple(width, tamano_box.height, InterpType.BILINEAR))

	def winPrincipal_show(self, s, archivo: str):
		if os.path.isfile(archivo):
			self.cargar_imagen_archivo(archivo)
		elif os.path.isdir(archivo):
			directorio = archivo if archivo.startswith('/') else os.path.abspath(archivo)
			...
			if imagenes:
				...
				DialogoError(self.winPrincipal, 'No se encontraron imágenes', f'{directorio} está vacío.')

	def mbtnPropiedades_clicked(self, sender):
		def foo(s, data):
			self.winPropiedades.hide()
			return True
		self.winPropiedades.connect('delete-event', foo)
		# self.winPropiedades.set_transient_for(win)
	...
	def mbtnIra_clicked(self, sender):
		...
		self.cargar_imagen_archivo(os.path.join(directorio, self.lista_imagenes[indice]))
	...
	def btnAgregarEtiqueta_clicked(self, sender):
		for e in self.entEtiqueta.get_text().split(','):
			e = e.strip()
			if e: self.agregar_etiqueta_en_listbox(e)
	...
	def mbtnAbrirCon_clicked(self, sender):
		...
		d = Gtk.AppChooserDialog.new(self.winPrincipal, Gtk.DialogFlags.MODAL, archivo)
	...
	def btnAnterior_clicked(self, sender):
		if self.pb_actual and self.archivonombre_actual:
			...
			cant = len(self.lista_imagenes)
			try:
				indice = self.lista_imagenes.index(os.path.basename(self.archivonombre_actual))
			except ValueError as ex:
				print(os.path.basename(self.archivonombre_actual))
			...
			if not self.cargar_imagen_archivo(os.path.join(directorio, self.lista_imagenes[indice]), mostrar_error=False):
				print(f'{self.lista_imagenes[indice]} no parece ser una imagen; ignorando...')
	...
	def btnAleatorio_clicked(self, sender):
		...
		while imagenes and not self.cargar_imagen_archivo(
			os.path.join(directorio, imagenes.pop(random.randint(0, len(imagenes) - 1))),
			mostrar_error=False
		):
			pass
	...
	def btnAgrandar_clicked(self, sender):
		if self.pb_actual and type(self.pb_actual) is Pixbuf:
			self.ajustar = False
			self.imgMain.set_from_pixbuf(self.pb_actual.scale_simple(self.imgMain.get_pixbuf().get_width()*self.FACTOR_ZOOM_MAS, self.imgMain.get_pixbuf().get_height()*self.FACTOR_ZOOM_MAS, InterpType.BILINEAR))
			self.tbtnAjustar.set_active(self.ajustar)

	def btnAchicar_clicked(self, sender):
		if self.pb_actual and type(self.pb_actual) is Pixbuf:
			self.ajustar = False
			self.imgMain.set_from_pixbuf(self.pb_actual.scale_simple(self.imgMain.get_pixbuf().get_width()*self.FACTOR_ZOOM_MENOS, self.imgMain.get_pixbuf().get_height()*self.FACTOR_ZOOM_MENOS, InterpType.BILINEAR))
			self.tbtnAjustar.set_active(self.ajustar)
	...
	def mbtnBorrar_clicked(self, sender):
		...
		if dialogo.respuesta:
			...
			shutil.move(self.archivonombre_actual, f'/tmpfs/.trash/{os.path.basename(self.archivonombre_actual)}')
			print(f'{self.archivonombre_actual} movido a /tmpfs/.trash/{os.path.basename(self.archivonombre_actual)}.')
	...
	def evboxImagen_button_press_event(self, s, ev):
		if self.pb_actual:
			if ev.button == 1:
				self.evboxImagen.get_window().set_cursor(Gdk.Cursor.new_from_name(Gdk.Display.get_default(), 'grabbing'))
			elif ev.button == 9:
				self.btnSiguiente_clicked(None)
			elif ev.button == 8:
				self.btnAnterior_clicked(None)
	...
	def evboxImagen_button_release_event(self, s, ev):
		...
		self.evboxImagen.get_window().set_cursor(Gdk.Cursor.new_from_name(Gdk.Display.get_default(), 'default'))
